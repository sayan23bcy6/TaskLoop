...
export const useTaskActions = ({
  ...
  const handleSubmitRating = async (taskId: string, rating: number) => {
    ...
    try {
      ...
      const isForCreator = isDoer; // If doer is rating, then it's for the creator
      ...
      if (existingRatings && existingRatings.length > 0) {
        // Rating already exists, don't insert again
        return true;
      }

      // First, store the individual rating
      ...
      if (isDoer) {
        // Update the creator's profile with the average rating
        await supabase
          .from('profiles')
          .update({
            requestor_rating: averageRating
          })
        ...
        // Update the doer's profile with the average rating
        await supabase
          .from('profiles')
          .update({
            doer_rating: averageRating
          })
      ...
      // Display toast only for the first time rating is submitted
      toast({
